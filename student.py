...
class Student:
    def __init__(self, root):
        ...
        # left label frame
        Left_frame = LabelFrame(main_frame, bd=2, relief=RIDGE, text="Studet Details",
        ...
        # current Course
        current_course_frame = LabelFrame(Left_frame, bd=2, relief=RIDGE, text="Current Course Informatin",
        ...
        dep_combo = ttk.Combobox(current_course_frame, textvariable=self.var_dep, font=("times new roman", 11, "bold"),
                                 width=17,
                                 state="readonly", )
        dep_combo["values"] = ("Select Depatrment", "C.S", "I.T", "B.C.A", "MECHNICAL", "CIVIL")
        dep_combo.current(0)
        ...
        course_combo = ttk.Combobox(current_course_frame, textvariable=self.var_course,
                                    ...
                                    state="readonly", )
        course_combo["values"] = ("Select Course", "B.E", "B.Tech", "B.S.C", "M.Tech", "M.E", "M.C.A", "M.S.C")
        ...
        # save
        save_btn = Button(btn_frame, text="Save", width=20, font=("times new roman", 10, "bold"), bg="blue", fg="white",
                          command=self.add_data)
        save_btn.grid(row=0, column=0)

        # update
        update_btn = Button(btn_frame, text="Update", width=20, font=("times new roman", 10, "bold"), bg="blue",
                            fg="white", command=self.update_data)
        update_btn.grid(row=0, column=1)

        # delete
        delete_btn = Button(btn_frame, text="Delete", width=20, font=("times new roman", 10, "bold"), bg="blue",
        ...
        # Take photo sample
        take_photo_btn = Button(btn_frame1, text="Take Photo Sample", width=42, font=("times new roman", 10, "bold"),
                                bg="blue",
        ...
        search_btn = Button(search_frame, text="Search", width=20, font=("times new roman", 10, "bold"), bg="red",
                            fg="white")
        search_btn.grid(row=0, column=2, padx=10, pady=0, sticky=W)

        showAll_btn = Button(search_frame, text="Show All", width=20, font=("times new roman", 10, "bold"), bg="red",
                             fg="white")
        ...
        self.student_table = ttk.Treeview(table_frame, column=(
            "id", "dep", "course", "sem", "name", "div", "roll", "gender", "dob", "email", "phone", "address",
            "teacher",
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                if update > 0:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        "update student set Dep=%s,Course=%s,Semester=%s,Name=%s,Division=%s,Roll=%s,Gender=%s,Dob=%s,Email=%s,Phone=%s,Address=%s,Teacher=%s,PhotoSample=%s,Year=%s where Student_id=%s",
                        (
                            self.var_dep.get(),
                            self.var_course.get(),
                            self.var_semester.get(),
